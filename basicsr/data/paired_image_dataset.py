...
from basicsr.data.data_util import paired_paths_from_folder2
...
class Dataset_PairedImage2(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.gt_folder, self.lq_folder = opt['dataroot_gt'], opt['dataroot_lq']
        if opt['load_mask']:
            self.mask_folder = opt['gt_mask']
            print('===================> masks are used')
        ...
        self.paths = paired_paths_from_folder2(
                [self.lq_folder, self.gt_folder, self.mask_folder], ['lq', 'gt', 'gt_mask'],
                self.filename_tmpl)
    ...
    def __getitem__(self, index):
        ...
        gt_mask_path = self.paths[index]['gt_mask_path']
        img_bytes = self.file_client.get(gt_mask_path, 'gt_mask')
        try:
            img_mask = imfrombytes(img_bytes, flag='color', float32=True)
        except:
            raise Exception("gt_mask path {} not working".format(gt_mask_path))
        ...
        if self.opt['phase'] == 'train':
            ...
            # padding
            img_gt, img_lq = padding(img_gt, img_lq, gt_size)
            _, img_mask = padding(img_gt, img_mask, gt_size)
            ...
            # img_gt, img_lq = paired_random_crop(img_gt, img_lq, gt_size, scale,
            #                                     gt_path)
            img_mask, img_lq, img_gt = paired_random_crop_DP(img_mask, img_lq, img_gt, gt_size, scale, gt_path)
        ...
        # BGR to RGB, HWC to CHW, numpy to tensor
        img_gt, img_lq, img_mask = img2tensor([img_gt, img_lq, img_mask],
                                    bgr2rgb=True,
                                    float32=True)
        # normalize
        if self.mean is not None or self.std is not None:
